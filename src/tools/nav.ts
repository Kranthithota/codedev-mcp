 ...
 * @param mtime - The modification date to format.
 * @returns A relative time string like "5m ago" or "2d ago".
 */
...
/**
 * Registers navigation tools for reading files and listing directory trees with metadata.
      ...
      if (params.path) {
        try {
          ...
          const cached = await cache.get<string>(
            `read:${params.path}:${params.start_line}:${params.end_line}`,
            filePath,
          );
          if (cached)
            return {
