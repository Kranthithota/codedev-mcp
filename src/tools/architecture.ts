...
export function registerArchitectureTools(server: McpServer) {
    ...
    server.registerTool(
        'architecture_check',
        {
            description: 'Check architectural constraints and patterns. Detects circular dependencies, layered architecture violations, and component isolation issues.',
            inputSchema: {
        ...
        async (params) => {
            try {
                ...
                return { content: [{ type: 'text', text: `architecture_check failed: ${error.message}` }], isError: true };
            }
        },
    );
            ...
            try {
                return await analytics.track('db_schema', async () => {
                    ...
                    if (result.migrations.length > 0) {
                        ...
                        result.migrations
                            .slice(0, 10)
                            .forEach((m: any) => lines.push(`  ${m.file}${m.description ? ` — ${m.description}` : ''}`));
                    }
                    return {
                        content: [{ type: 'text', text: lines.join('\n') }],
                        structuredContent: {
                            tables: result.tables.map((t: any) => ({ name: t.name, columns: t.columns?.length, source: t.source })),
                            relationships: result.relationships?.map((r: any) => ({ from: r.from, to: r.to, type: r.type })) || [],
                ...
                return { content: [{ type: 'text', text: `db_schema failed: ${error.message}` }], isError: true };
            }
        },
    );
            ...
            try {
                return await analytics.track('api_contracts', async () => {
                    ...
                    for (const ep of result.endpoints.slice(0, 50)) {
                        const params = ep.parameters?.map((p: any) => `${p.name}:${p.type}`).join(', ') || '';
                        lines.push(
                    ...
                    return {
                        content: [{ type: 'text', text: lines.join('\n') }],
                        structuredContent: {
                            endpoints: result.endpoints.map((ep: any) => ({ method: ep.method, path: ep.path, source: ep.source })),
                            totalEndpoints: result.endpoints.length,
                            sources: [...new Set(result.endpoints.map((ep: any) => ep.source))],
                            scannedPatterns: result.scannedPatterns || [],
                        },
                    };
                });
            } catch (error: any) {
                return { content: [{ type: 'text', text: `api_contracts failed: ${error.message}` }], isError: true };
            }
        },
    );
}
