    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                return await analytics.track('iac_analyze', async () => {
                    ...
                    return {
                        ...
                        structuredContent: {
                            ...
                            resources: result.resources?.map((r: any) => ({ type: r.type, name: r.name })) || [],
                            issues:
                                result.issues?.map((i: any) => ({ type: i.type || 'issue', message: i.message, severity: i.severity })) ||
                                [],
                ...
                return { content: [{ type: 'text', text: `iac_analyze failed: ${error.message}` }], isError: true };
            }
        },
    );
                ...
                return await analytics.track('cicd_analyze', async () => {
                    ...
                    for (const pipeline of result.pipelines) {
                        ...
                        for (const job of pipeline.jobs) {
                            ...
                            job.steps.slice(0, 5).forEach((s: any) => lines.push(`     → ${s}`));
                        }
                        if (pipeline.issues.length > 0) {
                            pipeline.issues.forEach((i: any) => lines.push(`  ${i.severity === 'error' ? '❌' : '⚠️'} ${i.message}`));
                        }
                        lines.push('');
                    }
                    return {
                        content: [{ type: 'text', text: lines.join('\n') }],
                        structuredContent: {
                            pipelines: result.pipelines.map((p: any) => ({ name: p.name, platform: p.platform, file: p.file })),
                            summary: result.summary || {},
                        },
                    };
                });
            } catch (error: any) {
                return { content: [{ type: 'text', text: `cicd_analyze failed: ${error.message}` }], isError: true };
            }
        },
    );
            ...
            try {
                return await analytics.track('monorepo_analyze', async () => {
                    ...
                    if (result.dependencyGraph.length > 0) {
                        ...
                        result.dependencyGraph.slice(0, 20).forEach((d: any) => lines.push(`  ${d.from} → ${d.to}`));
                    }
                    if (result.issues.length > 0) {
                        lines.push(`\n### Issues(${result.issues.length})`);
                        result.issues.forEach((i: any) => lines.push(`  ⚠️ ${i}`));
                    }
                    return {
                        content: [{ type: 'text', text: lines.join('\n') }],
                        structuredContent: {
                            type: result.type,
                            packages: result.packages.map((p: any) => ({ name: p.name, path: p.path, version: p.version })),
                            summary: result.summary || {},
                        },
                    };
                });
            } catch (error: any) {
                return { content: [{ type: 'text', text: `monorepo_analyze failed: ${error.message}` }], isError: true };
            }
        },
    );
}
