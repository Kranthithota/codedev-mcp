...
export function registerGitTools(server: McpServer) {
    ...
    server.registerTool(
        'git_log',
        {
            description: 'View git commit history. Filter by file, author, date range. Shows commit hash, date, author, and message.',
            inputSchema: {
            ...
            try {
                ...
                if (params.with_files) {
                    ...
                    const output = entries
                        .map((e) => `${e.hash.slice(0, 8)} ${e.date.split(' ')[0]} ${e.author}\n  ${e.message}${e.files?.length ? '\n  Files: ' + e.files.join(', ') : ''}`)
                        .join('\n\n');
                    return {
                        content: [{ type: 'text', text: output }],
                        structuredContent: { action: 'log', entries: entries.map((e) => ({ hash: e.hash, date: e.date, author: e.author, message: e.message })) },
                    };
                }
                ...
                const output = entries.map((e) => `${e.hash.slice(0, 8)} ${e.date.split(' ')[0]} ${e.author.padEnd(20)} ${e.message}`).join('\n');
                return {
                    content: [{ type: 'text', text: `Git log (${entries.length} commits):\n\n${output}` }],
                    structuredContent: { action: 'log', entries: entries.map((e) => ({ hash: e.hash, date: e.date, author: e.author, message: e.message })) },
                };
            } catch (error: any) {
                return { content: [{ type: 'text', text: `git_log failed: ${error.message}` }], isError: true };
            }
        },
    );
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                ...
                return { content: [{ type: 'text', text: `git_diff failed: ${error.message}` }], isError: true };
            }
        },
    );
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                ...
                const output = blame.map((b) => `${b.hash} ${b.date} ${b.author.padEnd(15)} L${String(b.line).padStart(4)} │ ${b.content}`).join('\n');
                return {
                    content: [{ type: 'text', text: output }],
                    structuredContent: { action: 'blame', entries: blame.map((b) => ({ hash: b.hash, author: b.author, line: b.line })) },
                };
            } catch (error: any) {
                return { content: [{ type: 'text', text: `git_blame failed: ${error.message}` }], isError: true };
            }
        },
    );
    ...
    server.registerTool(
        ...
        async () => {
            try {
                ...
                return { content: [{ type: 'text', text: `git_status failed: ${error.message}` }], isError: true };
            }
        },
    );
    ...
    server.registerTool(
        ...
        async () => {
            try {
                ...
                return {
                    content: [{ type: 'text', text: output }],
                    structuredContent: { action: 'branches', entries: branches.map((b) => ({ name: b, current: b === current })) },
                };
            } catch (error: any) {
                return { content: [{ type: 'text', text: `git_branches failed: ${error.message}` }], isError: true };
            }
        },
    );
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                ...
                return { content: [{ type: 'text', text: `git_show failed: ${error.message}` }], isError: true };
            }
        },
    );
        ...
        async (params) => {
            try {
                ...
                return {
                    ...
                    structuredContent: { action: 'contributors', entries: contribs.map((c) => ({ author: c.author, commits: c.commits })) },
                };
            } catch (error: any) {
                return { content: [{ type: 'text', text: `git_contributors failed: ${error.message}` }], isError: true };
            }
        },
    );
            ...
            try {
                ...
                return {
                    ...
                    structuredContent: {
                        ...
                        affected: impact.affectedFiles?.map((f: any) => ({ file: f.file || f, type: f.type || 'modified' })) || [],
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `change_impact failed: ${error.message}. Ensure this is a git repository or provide explicit file paths.`,
                        },
                    ],
                    isError: true,
                };
            ...
            try {
                return await analytics.track('branch_compare', async () => {
                    ...
                    if (result.added.length > 0) {
                        lines.push(`\n### Added Files`);
                        result.added.slice(0, 20).forEach((f: string) => lines.push(`  + ${f}`));
                    }
                    if (result.deleted.length > 0) {
                        lines.push(`\n### Deleted Files`);
                        result.deleted.slice(0, 20).forEach((f: string) => lines.push(`  - ${f}`));
                    }
                    if (result.modified.length > 0) {
                        lines.push(`\n### Modified Files`);
                        result.modified.slice(0, 30).forEach((f: string) => lines.push(`  ~ ${f}`));
                    }
                    if (result.renamed.length > 0) {
                        lines.push(`\n### Renamed Files`);
                        result.renamed.slice(0, 10).forEach((r: any) => lines.push(`  ${r.from} → ${r.to}`));
                    }
                    if (result.diffStat.length > 0) {
                        lines.push(`\n### Diff Stats`);
                        result.diffStat.slice(0, 20).forEach((d: any) => lines.push(`  ${d.file}: +${d.insertions} -${d.deletions}`));
                    }
                    return {
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `branch_compare failed: ${error.message}. Ensure you're in a git repository with the specified branches.`,
                        },
                    ],
                    isError: true,
                };
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                ...
                return { content: [{ type: 'text', text: `git_hooks failed: ${error.message}` }], isError: true };
            }
        },
    );
}
