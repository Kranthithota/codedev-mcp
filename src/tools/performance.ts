            ...
            try {
                return await analytics.track('perf_profile', async () => {
                    ...
                    if (result.bundleSize) {
                        ...
                        result.bundleSize.largest.forEach((b: any) => lines.push(`  ${b.name}: ${(b.size / 1024).toFixed(0)} KB`));
                        lines.push('');
                    }
                    ...
                    for (const [type, entries] of grouped) {
                        ...
                        for (const e of entries.slice(0, 10)) {
                            ...
                            lines.push(`  ${icon} ${e.name} — ${e.value} ${e.unit}${e.recommendation ? ` (${e.recommendation})` : ''}`);
                        }
                        lines.push('');
                    }
                    return {
                        ...
                        structuredContent: {
                            entries: result.entries.map((e: any) => ({ name: e.name, value: e.value, unit: e.unit })),
                            summary: result.summary || {},
                        },
                    };
                });
            } catch (error: any) {
                return { content: [{ type: 'text', text: `perf_profile failed: ${error.message}` }], isError: true };
            }
        },
    );
}
