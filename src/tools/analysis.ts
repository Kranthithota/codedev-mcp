            ...
            try {
                if (IS_MULTI_ROOT) {
                    ...
                    for (const root of ROOTS) {
                        const { stats, tree, summary } = await mapCodebase(root);
                ...
                const { stats, tree, summary } = await mapCodebase(CWD);
                return {
                    content: [
                        {
                            type: 'text',
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                ...
                if (params.language) files = files.filter((f) => detectLanguage(f) === params.language);
                files = files.slice(0, 300); // limit for performance
                ...
                analyses.sort((a, b) => {
                    ...
                    return (
                        (b as any)[sortKey === 'lines' ? 'codeLines' : sortKey] -
                        (a as any)[sortKey === 'lines' ? 'codeLines' : sortKey]
                    );
                });
                ...
                switch (params.action) {
                    case 'summary': {
                        ...
                        return {
                            ...
                            structuredContent: {
                                action: 'summary',
                                data: coverage.files.slice(0, 15).map((f: any) => ({
                                    file: f.file,
                    ...
                    case 'file': {
                        ...
                        if (fc.uncoveredFunctions.length > 0) output += `  Untested functions: ${fc.uncoveredFunctions.join(', ')}\n`;
                        return {
                            content: [{ type: 'text', text: output }],
                            structuredContent: {
                                action: 'file',
